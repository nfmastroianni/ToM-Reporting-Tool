...
function scanData() {
  ...
  if (sheets.length == 1) {
    ...
    if (dataSheet.getRange(3,1).getValue() == "Filters") {
      dataSheet.deleteRow(3); // This line was added due to LinkIt! adding a row to list filters used in generating their report
    }
    var numStudents = dataSheet.getLastRow()-4;
    var colD = dataSheet.getRange(4,4).getValue();
    dataSheet.getRange("E5:Q").setNumberFormat("@");
    var reportType;
    if (colD == "Result Date") {
      var obj = {
        type : "Class",
...
function classAdjust() {
  ...
  sheet.getRange(4, 6).setValue("Class");
...
function makeSheets(teacher) {
  ...
  var studentIDs = [];
  var studentNames = [];
  var nameDataRange = dataSheet.getRange(5, 1, numStudents, 7);
  var nameData = nameDataRange.getValues();
  for (i = 0; i < nameData.length; i++) {
    var studentName = nameData[i][0] + nameData[i][1];
    if(studentNames.indexOf(studentName) < 0) {
      ...
      var d = new Date(nameData[i][6]);
      var aDate = d.toLocaleDateString();
      ss.insertSheet(nameData[i][0] + ', ' + nameData[i][1] + ' ' + aDate, i+1);
      var currentSheet = ss.getSheetByName(nameData[i][0] + ', ' + nameData[i][1] + ' ' + aDate).setColumnWidths(1, 10, 87);
    }
...
function buildEmptyReport(count,grade) {
  ...
  if (sheet.getSheetName() != "studentResults") {
    var currentID = sheet.getRange("B4").getValue();
    var currentRow = count + 5;
    var rawLang = dataSheet.getRange(currentRow, 29).getValue();
    var requestedLanguage = rawLang.trim();
    var templateSheets = template.getSheets();
...
function finishReport(count) {
  ...
  if (sheet.getSheetName() != "studentResults" && sheet.getSheetName() != "DemoReport") {
    ...
    for (i=0; i<socialEmotional[0].length; i++) {
      ...
      var column;
      if (Number(socialEmotional[0][i].valueOf()) == 0){
        column = 1;
      } else if (Number(socialEmotional[0][i].valueOf()) == 1) {
        column = 4;
      } else if (Number(socialEmotional[0][i].valueOf()) == 2) {
        column = 7;
  ...
  for (var col in cols) {
    ...
    if (grade == 4 && col < 6) {
      for (var rate in ratings) {
        if(rate == 0) {
          formulaArray.push("=(COUNTIF(dataCopy!" + cols[col] + "2:" + cols[col] + ", \"=" + ratings[rate] + "\")+COUNTIF(dataCopy!"+cols[col] + "2:" + cols[col] + ", \"=1\"))/COUNT(dataCopy!" + cols[col] + "2:" + cols[col] + ")");
        } else if (rate == 1) {
          formulaArray.push("=(COUNTIF(dataCopy!" + cols[col] + "2:" + cols[col] + ", \"=2" + "\")/COUNT(dataCopy!" + cols[col] + "2:" + cols[col] + "))");
        }
          else {
          formulaArray.push("=0");
        }
...
function convertData(grade){
  ...
  sheet.getRange(5, 7, lastRow).setNumberFormat("mm-dd-yyyy");
  return grade;
}
